...
import torchsnooper
'''
block, layers, modality='RGB',shortcut_type='B', num_classes=400,dropout=0.5,ST_struc=('A','B','C')
'''

__all__ = ['P3D', 'P3D63', 'P3D131','P3D199']    #input[a,b,c,d,e] a:picture list b:picture channel c:chip length d:height e:width
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,n_s=0,depth_3d=47,ST_struc=('A','B','C'),reduction=16):
    ...
    def forward(self, x):
        ...
        # print(residual.shape,self.se_module(out).shape)
...
class SEModule(nn.Module):
    ...
    def forward(self, x):
        module_input = x
        if len(module_input.size()) == 4:
            x = self.avg_pool2d(x)
        elif len(module_input.size()) == 5:
            x = self.avg_pool3d(x).squeeze(3)
        else:
            assert False
        # x = self.avg_pool(x)
        x = self.fc1(x)
        ...
        if len(module_input.size()) == 5:
            x = x.unsqueeze(3)
        return module_input * x

class SE_strip(nn.Module):
    def __init__(self,c):
        super(SE_strip,self).__init__()
...
class P3D(nn.Module):
    def __init__(self, block, layers, modality='RGB',
        ...
        # self.conv5 = nn.Conv2d(512 * block.expansion, 512 * block.expansion//2, kernel_size=(5,5), stride=1, padding=(0,1), bias=False)
        ...
        self.feat_attention = SE_strip(512 * block.expansion)
